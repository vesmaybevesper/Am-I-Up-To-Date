...
import dev.vesper.aiutd.common.ChangelogOpeners;import dev.vesper.aiutd.common.config.Config;import dev.vesper.aiutd.common.config.EndUserConfig;
...
public class FabricClientEntrypoint implements ClientModInitializer {
	...
	public void onInitializeClient() {
		ClientCommandRegistrationCallback.EVENT.register(((dispatcher, unused) -> {
			...
			//? if >=26.1 {
			dispatcher.register(ClientCommands.literal("rinkuOpen").executes(context -> {
				ChangelogOpeners.rinku();
				return 1;
			}));
			//?} else {
			/*dispatcher.register(ClientCommandManager.literal("rinkuOpen").executes(context -> {
				ChangelogOpeners.rinku();
				return 1;
			}));
			*///?}
